...
const validateAddresses = (): void => {
    if (process.env.PROXY_WALLET && !isValidEthereumAddress(process.env.PROXY_WALLET)) {
        ...
        console.error('PROXY_WALLET is not a valid Ethereum address.');
        ...
        throw new Error('Invalid PROXY_WALLET address format');
    }
    ...
    if (
        ...
        console.error('USDC_CONTRACT_ADDRESS is not a valid Ethereum address.');
        ...
        throw new Error('Invalid USDC_CONTRACT_ADDRESS format');
    }
};
...
const validateUrls = (): void => {
    if (process.env.CLOB_HTTP_URL && !process.env.CLOB_HTTP_URL.startsWith('http')) {
        ...
        console.error('CLOB_HTTP_URL must start with http:// or https://');
        ...
        throw new Error('Invalid CLOB_HTTP_URL: Must be a valid HTTP/HTTPS URL.');
    }

    if (process.env.CLOB_WS_URL && !process.env.CLOB_WS_URL.startsWith('ws')) {
        console.error('\n❌ Invalid CLOB_WS_URL\n');
        console.error('CLOB_WS_URL must start with ws:// or wss://');
        console.error('Default value: wss://ws-subscriptions-clob.polymarket.com/ws\n');
        console.error('⚠️  Use the default value unless you have a specific reason to change it!\n');
        throw new Error('Invalid CLOB_WS_URL: Must be a valid WebSocket URL (ws:// or wss://).');
    }

    if (process.env.RPC_URL && !process.env.RPC_URL.startsWith('http')) {
        console.error('\n❌ Invalid RPC_URL\n');
        console.error('RPC_URL must start with http:// or https://\n');
        ...
        throw new Error('Invalid RPC_URL: Must be a valid HTTP/HTTPS URL.');
    }
    ...
    if (process.env.MONGO_URI && !process.env.MONGO_URI.startsWith('mongodb')) {
        ...
        throw new Error('Invalid MONGO_URI: Must be a valid MongoDB connection string.');
    }
};
            ...
            if (Array.isArray(parsed)) {
                ...
                for (const addr of addresses) {
                    if (!isValidEthereumAddress(addr)) {
                        ...
                        console.error('One or more addresses are not valid Ethereum addresses.');
                        console.error('Expected format: 0x followed by 40 hexadecimal characters\n');
                        ...
                        throw new Error('Invalid Ethereum address in USER_ADDRESSES');
                    }
                }
                return addresses;
            }
